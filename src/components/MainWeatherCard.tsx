...
import type { DailyWeather, Location, WeatherData } from "../types";
...
interface Props {
    ...
    selectedDaily?: DailyWeather | null;
    selectedDetail?: WeatherData | null;
}

const MainWeatherCard: React.FC<Props> = ({
                                              weatherData,
                                              loading,
                                              setLocation,
                                              selectedDaily = null,
    ...
    const showingDaily = !!(selectedDaily && selectedDetail);
        ...
        <div className="bg-white shadow-2xl flex-1 flex flex-col justify-center items-center p-8 min-h-[28rem]">
            ...
            <div className="flex justify-center mb-12">
                <div className="relative w-40 h-40 flex items-center justify-center">
                    <WeatherIcon icon={(showingDaily ? selectedDetail!.icon : weatherData.icon)} size={160} />
                </div>
            </div>
                ...
                {showingDaily ? (
                    <>
                        <div className="text-7xl font-light text-gray-800 mb-3">
                            {selectedDetail!.temperature.toFixed(1)}°
                        </div>
                        <div className="text-gray-500 text-xl mb-2">
                            {selectedDetail!.condition}
                        </div>
                        <div className="text-gray-400 text-lg mb-4">
                            Feels like {selectedDetail!.feelsLike.toFixed(1)}°
                        </div>
                        <div className="flex justify-center gap-6 text-sm text-gray-500 mb-4">
                            <div className="flex items-center gap-1">
                                <Droplets size={16} className="text-blue-400" />
                                <span>{selectedDetail!.humidity}%</span>
                            </div>
                            <div className="flex items-center gap-1">
                                <Wind size={16} className="text-gray-400" />
                                <span>{selectedDetail!.windSpeed.toFixed(1)} km/h</span>
                            </div>
                        </div>
                    </>
                ) : (
